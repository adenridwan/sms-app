...
export default function Reports() {
    ...
    const loadDashboard = useCallback(async () => {
        setLoading(true);
        try {
            const response = await payrollReportsApi.dashboard<DashboardData>({ year: parseInt(dashboardYear) });
    ...
    const loadMonthlyRecap = useCallback(async () => {
        setLoading(true);
        try {
            const response = await payrollReportsApi.monthlyRecap<MonthlyRecapItem>({ year: parseInt(recapYear) });
            setMonthlyRecapData(response.data.data || []);
            setMonthlyRecapSummary((response.data.summary as typeof monthlyRecapSummary) ?? null);
        } catch (error) {
    ...
    const loadPph21 = useCallback(async () => {
        setLoading(true);
        try {
            const response = await payrollReportsApi.pph21<Pph21Item>({
            ...
            setPph21Summary((response.data.summary as typeof pph21Summary) ?? null);
            setPph21MonthlyBreakdown((response.data.monthly_breakdown as typeof pph21MonthlyBreakdown) ?? []);
            setPph21Meta(response.data.meta ?? null);
        } catch (error) {
    ...
    const loadBpjs = useCallback(async () => {
        setLoading(true);
        try {
            const response = await payrollReportsApi.bpjs<BpjsItem>({
                year: parseInt(bpjsFilters.year),
                month: bpjsFilters.month ? parseInt(bpjsFilters.month) : undefined,
                per_page: 15,
            });
            setBpjsData(response.data.data || []);
            setBpjsSummary((response.data.summary as typeof bpjsSummary) ?? null);
            setBpjsMonthlyBreakdown((response.data.monthly_breakdown as typeof bpjsMonthlyBreakdown) ?? []);
            setBpjsMeta(response.data.meta ?? null);
        } catch (error) {
